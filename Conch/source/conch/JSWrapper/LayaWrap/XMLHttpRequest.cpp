    ...
    */
    ...
    void _onPostComplete_JSThread(XMLHttpRequest* pxhr, char* p_Buff, int p_nLen, bool p_bBin, std::weak_ptr<int> cbref) 
    {
        ...
        if (p_Buff) 
        {
            if (p_bBin) 
            {
                ...
                pxhr->m_jsfunPostComplete.Call(ab, p_Buff); //执行JS回调函数
            }
            else 
            {
                std::string strBuff;
                ...
                pxhr->m_jsfunPostComplete.Call(strBuff); //执行JS回调函数
            }
            delete p_Buff;
        }
        else 
            ...
            const std::string&, const std::string&,int curlr, int httpr, const std::string&,std::weak_ptr<int> callbackref) 
    {
        ...
        else
        {
            ...
            pPoster->postToJS(std::bind(_onPostComplete_JSThread, pxhr, pBuff, len, bBin, callbackref)); //调用方法_onPostComplete_JSThread
        }
    }
    /*
